  ...
  void build(vector<vector<int>>& v) {
    for (int i = 1; i <= n; ++i) {
      ...
      for (int l = 1; l <= logM; ++l) {
        for (int j = 1; j + (1 << l) - 1 <= m; ++j) {
          sTable[i][0][j][l] = max(sTable[i][0][j][l - 1], sTable[i][0][j + (1 << (l - 1))][l - 1]);
        }
      }
    }
    for (int k = 1; k <= logN; ++k) {
      for (int i = 1; i + (1 << k) - 1 <= n; ++i) {
        for (int l = 1; l <= logM; ++l) {
          for (int j = 1; j + (1 << l) - 1 <= m; ++j) {
            sTable[i][k][j][l] = max(sTable[i][k - 1][j][l], sTable[i + (1 << (k - 1))][k - 1][j][l]);
          }
        }
      }
    }
...
public:
  ...
  int query(int x1, int y1, int x2, int y2) {
    ...
    int mxR1 = max(sTable[x1][logX][y1][logY], sTable[x1][logX][y2 - (1 << logY) + 1][logY]);
    int mxR2 = max(sTable[x2 - (1 << logX) + 1][logX][y1][logY], sTable[x2 - (1 << logX) + 1][logX][y2 - (1 << logY) + 1][logY]);
